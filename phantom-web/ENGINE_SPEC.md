# ⚙️ Phantom-Web 核心引擎规范 (V12.1)

> 版本: V12.1-Phantom (JS 前端) | 引擎代号: 青羽
...
### 5.3 Phantom-Pack 打包格式

将 `.ptm` 图纸 + 所有加密分片合并为单一 `.phantom` 文件，用于邮箱传输。

**格式定义**：
```
偏移  | 大小    | 内容
0     | 4       | Magic: "PHNT" (0x50484E54)
4     | 1       | Version: 0x01
5     | 4       | Manifest Length (Uint32 LE)
9     | M       | Manifest Data (.ptm 二进制)
9+M   | 4       | Chunk Count (Uint32 LE)
9+M+4 | 可变    | Chunk 列表:
      | 2       |   Chunk ID Length (Uint16 LE)
      | L       |   Chunk ID (UTF-8)
      | 4       |   Chunk Data Length (Uint32 LE)
      | D       |   Chunk Data
```

**实现**：[`phantom-pack.js`](../src/storage/phantom-pack.js)

**注意**：打包是纯输出操作，解密端无需感知 `.phantom` 格式。用户收到邮件后下载附件，解包得到 `.ptm` 图纸，按现有解密流程操作。
...
**问题**: Go 端 `os.ReadFile(filePath)` 将整个文件读入内存。50GB 文件 → 50GB 堆分配 → OOM。主线程循环 `data[start:end]` 切片分配后抛给 goroutine，内存无法复用。

**修复**: Go 端使用 `os.Open` + `file.ReadAt` + `sync.Pool` 三重保障。详见 [`phantom-core-go/ENGINE_SPEC.md`](../phantom-core-go/ENGINE_SPEC.md#92-陷阱2go-大文件oom已修复)。
...
### 9.4 陷阱4：`.phantom` 文件解密时 fileId 丢失（已修复）

**问题**: `_handleManifestUpload` 处理 `.phantom` 打包文件时，chunks 以原始 `fileId/0000000X` 格式写入 provider，但 `_currentFileId` 未设置。解密时 fallback 到 `'default'`，导致 `toChunkId('default', i)` 生成 `default/0000000X`，与写入的 key 不匹配，所有分片下载失败。

**修复**: 从第一个 chunkId 中通过 `parseChunkId()` 提取 `fileId`：
...
- **问题**: Session 目录（`chunksDir`）在加密循环**之后**才创建，Provider 初始化时指向旧的 `*storeDir`。分片写入旧目录后，代码尝试重新创建 Provider 指向新目录，但分片不会自动迁移。
- **修复**: 将 Session 目录创建移到 Provider 初始化之前，Provider 直接指向 `chunksDir`。

**Bug #15** 🟡 Medium — Go `detectFileID` 与 `LocalFileProvider` 不匹配
- **问题**: `LocalFileProvider` 使用 `{baseDir}/{fileId}/{index}.chunk` 子目录结构，`detectFileID` 在 `chunksDir` 中查找子目录名作为 fileId。两者逻辑一致，**实际兼容**，无需修改。

**Bug #16** 🟡 Medium — Go `fileSize int` 溢出
- **问题**: `fileSize := int(fi.Size())` 在 32 位系统上溢出。`Manifest.FileSize` 改为 `int64` 后 `manifest.go:165` 编译器报错。
- **修复**: `manifest.go:165` 改为 `int64(fileSizeHi<<32 | fileSizeLo)`；`main.go:127` 改为 `fileSize := fi.Size()`；`verify-proof/main.go` 中三处 `len(data)` 改为 `int64(len(data))`。
...
- **问题**: `MaxFileSize()` 返回 `0xFFFFFFFFFFFF`（256TB-1），语义正确。**确认无问题**。

### 9.5 陷阱5：`encryptFile` 进度回调 `bytesProcessed` 溢出（已修复）

**问题**: `bytesProcessed: (i + 1) * chunkSize` 在最后一个 chunk 会超过 `file.size`。例如 6MB 文件、5MB chunkSize，最后一个 chunk 的 `bytesProcessed = 10MB > 6MB`，进度条可能超过 100%。

**修复**: 使用 `Math.min((i + 1) * chunkSize, file.size)` 确保不超过文件总大小。

### 9.6 陷阱6：`ConcurrencyPool` 重试次数语义偏差（已修复）

**问题**: `_executeWithRetry` 中 `for (let attempt = 0; attempt < this._maxRetries; attempt++)` 当 `_maxRetries=3` 时实际执行 3 次（初始 + 2 次重试），但语义上 `_maxRetries` 应表示"总尝试次数"。第 3 次失败后静默吞掉错误，比预期少一次重试机会。

**修复**: 更新注释明确语义为"总尝试次数 = maxRetries（初始执行 + 重试）"，消除歧义。
